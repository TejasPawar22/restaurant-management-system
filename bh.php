...
<body>
    <div id="foem-1">
        ...
        <form action="" method="post">
          <?php
          $qry1 = "SELECT * FROM `hall_details` WHERE date='$h'";

          $run = mysqli_query($sql, $qry1);
          $row = mysqli_num_rows($run);
          if ($row > 0) { ?>
        <tr>
                  <td>Status</td>
                  <td><input type="text" name="fail" value="Sorry Hall Is Not Available" class="r" disabled="disabled" title="Status"></td>
  ...
  <tr>
            ...
            if ($row > 0) { ?>
                ...
                <tr>
                  <td>Booking Date:</td>
                  <td><input type="date" name="date" value="<?php echo $h; ?>" readonly="true"   title="Check in" required></td>
          ...
          ?>
            <?php if (isset($_POST['submit'])) {
                ...
                $qry = "INSERT INTO `hall_details`(`name`, `address`, `state`, `city`, `email`, `date`, `members`, `function`) VALUES ('$hname','$haddress','$hstate','$hcity','$hemail','$hdate','$hmembers','$function')";
                $run = mysqli_query($sql, $qry);
                if ($run) { ?>
                        <script>
                            alert("Details Saved Successfully");
                            window.open('cartpayment2.php','_self');
                        </script>
